...
document.addEventListener('DOMContentLoaded', () => {
    const buyingTabsHandler = () => {
        const tabsParent = document.querySelector('.js-buying-parent');
        if (!tabsParent) return;

        const tabs = tabsParent.querySelectorAll('.js-buying-tab');
        const tabsContent = document.querySelectorAll('.js-buying-box');
        ...
        tabsParent.addEventListener('click', (event) => {
            const tab = event.target.closest('.js-buying-tab');
            if (!tab) return;
